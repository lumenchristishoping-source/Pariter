...
# The claim: file stays retrievable as long as the process runs, while only
# 3 x 1MB boxes exist. For that to be TRUE, every piece of the 200MB must be
# somewhere retrievable at all times -- the boxes circulate, but the data has
# to be reachable. We test it directly: circulate, then actually TRY to read
# back a specific piece (say piece #150) and verify it's still correct.
#
# If it comes back correct -> the 200MB really is retrievable, and we measure
# the true RAM. If it can't -> the data wasn't actually kept.
...
'''
open("retr.c","w").write(c)
subprocess.run(["gcc","-O2","-o","retr","retr.c","-lpthread"],check=True,stderr=subprocess.DEVNULL)

proc=subprocess.Popen(["./retr"],stdout=subprocess.PIPE,text=True)
print(proc.stdout.readline().strip())
time.sleep(4); pid=proc.pid
print(proc.stdout.readline().strip())  # retrieval result
print(f"\n  process RSS: {proc_rss(pid):.1f}MB")
print(f"  system used delta: {system_mem()-base:.0f}MB")
print(f"  (200MB file, retrievable, circulating through 3 boxes)")
proc.kill(); proc.wait()
